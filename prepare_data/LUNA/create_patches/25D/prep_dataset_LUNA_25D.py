...
Important: make sure that all the required csv files are correctly labeled
and saved in its directory.
"""
...
dir_images = os.path.join(Directory_resampled, "images")
# out directorys
dir_out = os.path.join(Directory_resampled, "final_dataset")
...
# out csvs
path_failed_annotations = os.path.join(dir_out, "nonodules_annotations.csv")
    ...
    if filename.endswith(".nii.gz") and filename[0] != ".":
        ...
        for org in nodules_origins_sizes:
            ...
            for k in range(N_patches-2):
                ...
                patch_minus, patch_base, patch_plus = create_patches.luna_create_wnod_patch_context(org_v,
                                                                                                    N_patches,
                                                                                                    max_shift,
                                                                                                    image,
                                                                                                    sz2,
                ...
                # Minus
                patch_name_minus = (
                    filename_no_ext + "_" + alphabet[r] + str(s + 1)
                    + '_minus' + ".nii.gz"
                )
                patch_path_minus = os.path.join(dir_out_patches,
                                                patch_name_minus)
                patch_path_regress_minus = os.path.join(
                                    dir_out_patches_regress, patch_name_minus
                                    )
                # Base
                patch_name_base = (
                    filename_no_ext + "_" + alphabet[r] + str(s + 1)
                ...
                patch_path_base = os.path.join(dir_out_patches,
                                               patch_name_base)
                patch_path_regress_base = os.path.join(
                                    dir_out_patches_regress, patch_name_base
                                    )
                # Plus
                patch_name_plus = (
                    filename_no_ext + "_" + alphabet[r] + str(s + 1)
                ...
                patch_path_plus = os.path.join(dir_out_patches,
                                               patch_name_plus)

                patch_path_regress_plus = os.path.join(
                                    dir_out_patches_regress, patch_name_plus
                                    )
                ...
                # Write Patch
                sitk.WriteImage(patch_minus, patch_path_minus)
                sitk.WriteImage(patch_minus, patch_path_regress_minus)
                sitk.WriteImage(patch_base, patch_path_base)
                sitk.WriteImage(patch_base, patch_path_regress_base)
                sitk.WriteImage(patch_plus, patch_path_plus)
                sitk.WriteImage(patch_plus, patch_path_regress_plus)
        ...
        for k in range(N_rand):
            ...
            patch_minus, patch_base, patch_plus, rand_o = create_patches.luna_create_random_patch(nodules_origins_sizes,
                                                                    window_width,
                                                                    Size,
                                                                    image,
                                                                    nodules_size_z,
